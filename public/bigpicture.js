...
var bigpicture = (function() {
  ...
  function newText(x, y, size, text, id) {
    var tb = document.createElement('div');
    tb.className = "text";
    tb.contentEditable = true;
    tb.innerHTML = text;
    tb.id = id || ('' + Math.random()).slice(2);
  ...
  bpContainer.onclick = function(e) {
    ...
    var tb = newText(current.x + (e.clientX) * current.zoom, current.y + (e.clientY) * current.zoom, 20 * current.zoom, '').focus();
    publish(tb);
  };
  ...
  var debouncePublish = debounce(publish, 500)
 ...
 */
bigpicture.subscribe(function(data) {
  console.log(data);
  $.ajax({
    url: '/update',
    type: 'POST',
...
$.getJSON('/text', function (data) {
  data.map(function(data) {
    ...
    if ($tb.length) {
      ...
      var tb = bigpicture.push(data.x, data.y, data.size, data.text, data.id);
    }
  });
});
